...
# -------------------------------------------------
  ...
  mr1F[1] <- mean.mr1F
  mb1F[1] <- mean.mb1F  
  log(mb2F[1]) <- mu.mb1F + ep.mb1F[1] # Background mortality hazard rate subadult female   
  log(mr2F[1]) <- mu.mr2F + ep.mr2F[1] # road-kill mortality hazard rate subadult female
  mr3F[1] <- mean.mr3F
  mb3F[1] <- mean.mb3F  
  # Survival
  s1F[1] <- exp(-(mr1F[1] + mb1F[1])) 
  s2F[1] <- exp(-(mr2F[1] + mb2F[1]))
  s3F[1] <- exp(-(mr3F[1] + mb3F[1]))
  # Roadkill mortality
  rk1F[1] <- (1 - s1F[1]) * (mr1F[1] / (mr1F[1] + mb1F[1]))
  rk2F[1] <- (1 - s2F[1]) * (mr2F[1] / (mr2F[1] + mb2F[1]))
  rk3F[1] <- (1 - s3F[1]) * (mr3F[1] / (mr3F[1] + mb3F[1]))
  # Background mortality
  b1F[1]  <- (1 - s1F[1]) * (mb1F[1] / (mr1F[1] + mb1F[1]))
  b2F[1]  <- (1 - s2F[1]) * (mb2F[1] / (mr2F[1] + mb2F[1]))
  b3F[1]  <- (1 - s3F[1]) * (mb3F[1] / (mr3F[1] + mb3F[1]))
  # Random effects
  ep.mr1F[1] <- 0
  ep.mb1F[1] <- 0	
  ep.mr2F[1] ~ T(dnorm(0, sd=sigma.mr2F),-10,10)
  ep.mb2F[1] ~ T(dnorm(0, sd=sigma.mb2F),-10,10)
  ep.mr3F[1] <- 0
  ep.mb3F[1] <- 0  
  # MALES
  #~~~~~~~~~~ 
  ...
  log(mb2M[1]) <- mu.mb2M + ep.mb2M[1] # Background mortality hazard rate subadult male
  mr3M[1] <- mean.mr3M
  mb3M[1] <- mean.mb3M
  # Survival 
  s1M[1] <- exp(-(mr1M[1] + mb1M[1])) 
  s2M[1] <- exp(-(mr2M[1] + mb2M[1]))
  s3M[1] <- exp(-(mr3M[1] + mb3M[1]))
  # Roadkill mortality  
  rk1M[1] <- (1 - s1M[1]) * (mr1M[1] / (mr1M[1] + mb1M[1]))
  rk2M[1] <- (1 - s2M[1]) * (mr2M[1] / (mr2M[1] + mb2M[1]))
  rk3M[1] <- (1 - s3M[1]) * (mr3M[1] / (mr3M[1] + mb3M[1]))
  # Background mortality
  b1M[1]  <- (1 - s1M[1]) * (mb1M[1] / (mr1M[1] + mb1M[1]))
  b2M[1]  <- (1 - s2M[1]) * (mb2M[1] / (mr2M[1] + mb2M[1]))
  b3M[1]  <- (1 - s3M[1]) * (mb3M[1] / (mr3M[1] + mb3M[1]))
  # Random effects
  ep.mr1M[1] <- 0
  ep.mb1M[1] <- 0
  ep.mr2M[1] ~ T(dnorm(0, sd=sigma.mr2M),-10,10)
  ep.mb2M[1] ~ T(dnorm(0, sd=sigma.mb2M),-10,10)
  ep.mr3M[1] <- 0
  ep.mb3M[1] <- 0
  ...
  for (t in 2:(n.occasions-1)){  
    ...
    log(mb1F[t]) <- mu.mb1F + ep.mb1F[t] # Background mortality hazard rate juv    
    log(mr2F[t]) <- mu.mr2F + ep.mr2F[t] # road-kill mortality hazard rate subadult
    log(mb2F[t]) <- mu.mb2F + ep.mb2F[t] # Background mortality hazard rate subadult
    log(mr3F[t]) <- mu.mr3F + ep.mr3F[t] # road-kill mortality hazard rate adult
    log(mb3F[t]) <- mu.mb3F + ep.mb3F[t] # Background mortality hazard rate adult
    # Survival	   
    s1F[t] <- exp(-(mr1F[t] + mb1F[t])) 
    s2F[t] <- exp(-(mr2F[t] + mb2F[t]))
    s3F[t] <- exp(-(mr3F[t] + mb3F[t]))
    # Roadkill mortality  	
    rk1F[t] <- (1 - s1F[t]) * (mr1F[t] / (mr1F[t] + mb1F[t]))
    rk2F[t] <- (1 - s2F[t]) * (mr2F[t] / (mr2F[t] + mb2F[t]))
    rk3F[t] <- (1 - s3F[t]) * (mr3F[t] / (mr3F[t] + mb3F[t]))
    # Background mortality    
    b1F[t]  <- (1 - s1F[t]) * (mb1F[t] / (mr1F[t] + mb1F[t]))
    b2F[t]  <- (1 - s2F[t]) * (mb2F[t] / (mr2F[t] + mb2F[t]))
    b3F[t]  <- (1 - s3F[t]) * (mb3F[t] / (mr3F[t] + mb3F[t]))
    # Random effects	
    ep.mr1F[t] ~ T(dnorm(0, sd=sigma.mr1F),-10,10)
    ep.mb1F[t] ~ T(dnorm(0, sd=sigma.mb1F),-10,10)	
    ep.mr2F[t] ~ T(dnorm(0, sd=sigma.mr2F),-10,10)
    ep.mb2F[t] ~ T(dnorm(0, sd=sigma.mb2F),-10,10)
    ep.mr3F[t] ~ T(dnorm(0, sd=sigma.mr3F),-10,10)
    ep.mb3F[t] ~ T(dnorm(0, sd=sigma.mb3F),-10,10)

    # MALES
    #~~~~~~~~~~
    # Survival
    log(mr1M[t]) <- mu.mr1M + ep.mr1M[t] # road-kill mortality hazard rate juv
    log(mb1M[t]) <- mu.mb1M + ep.mb1M[t] # Background mortality hazard rate juv   
    log(mr2M[t]) <- mu.mr2M + ep.mr2M[t] # road-kill mortality hazard rate subadult
    log(mb2M[t]) <- mu.mb2M + ep.mb2M[t] # Background mortality hazard rate subadult
    log(mr3M[t]) <- mu.mr3M + ep.mr3M[t] # road-kill mortality hazard rate adult
    log(mb3M[t]) <- mu.mb3M + ep.mb3M[t] # Background mortality hazard rate adult    
    # Survival	    
	s1M[t] <- exp(-(mr1M[t] + mb1M[t])) 
    s2M[t] <- exp(-(mr2M[t] + mb2M[t]))
    s3M[t] <- exp(-(mr3M[t] + mb3M[t]))
    # Roadkill mortality  
	rk1M[t] <- (1 - s1M[t]) * (mr1M[t] / (mr1M[t] + mb1M[t]))
    rk2M[t] <- (1 - s2M[t]) * (mr2M[t] / (mr2M[t] + mb2M[t]))
    rk3M[t] <- (1 - s3M[t]) * (mr3M[t] / (mr3M[t] + mb3M[t]))
    # Background mortality    
	b1M[t]  <- (1 - s1M[t]) * (mb1M[t] / (mr1M[t] + mb1M[t]))
    b2M[t]  <- (1 - s2M[t]) * (mb2M[t] / (mr2M[t] + mb2M[t]))
    b3M[t]  <- (1 - s3M[t]) * (mb3M[t] / (mr3M[t] + mb3M[t]))
    # Random effects	
    ep.mr1M[t] ~ T(dnorm(0, sd=sigma.mr1M),-10,10)
    ep.mb1M[t] ~ T(dnorm(0, sd=sigma.mb1M),-10,10)	
    ep.mr2M[t] ~ T(dnorm(0, sd=sigma.mr2M),-10,10)
    ep.mb2M[t] ~ T(dnorm(0, sd=sigma.mb2M),-10,10)
    ep.mr3M[t] ~ T(dnorm(0, sd=sigma.mr3M),-10,10)
    ep.mb3M[t] ~ T(dnorm(0, sd=sigma.mb3M),-10,10)
  }
  ...
  mean.mr1F ~ dunif(0.001, 3)
  sigma.mr1F ~ T(dnorm(0, 0.1),0,)
  
  mu.mr1M <- log(mean.mr1M)     
  mean.mr1M ~ dunif(0.001, 3)
  sigma.mr1M ~ T(dnorm(0, 0.1),0,)
  
  mu.mb1F <- log(mean.mb1F)
  mean.mb1F ~ dunif(0.001, 3)
  sigma.mb1F ~ T(dnorm(0, 0.1),0,)
  
  mu.mb1M <- log(mean.mb1M)
  mean.mb1M ~ dunif(0.001, 3)
  sigma.mb1M ~ T(dnorm(0, 0.1),0,)
  ...
  mu.mb2F <- log(mean.mb2F)
  mean.mb2F ~ dunif(0.001, 3)
  sigma.mb2F ~ dunif(0,5)
  
  mu.mb2M <- log(mean.mb2M)
  mean.mb2M ~ dunif(0.001, 3)
  sigma.mb2M ~ dunif(0,5)   
  ...
  mu.mb3F <- log(mean.mb3F)
  mean.mb3F ~ dunif(0.001, 3)
  sigma.mb3F ~ dunif(0,5)
  
  mu.mb3M <- log(mean.mb3M)
  mean.mb3M ~ dunif(0.001, 3)
  sigma.mb3M ~ dunif(0,5)
  ...
  mean.EF <- mean(EMF[1:n.occasions]) 
  ...
  for (t in 1:n.occasions) {
    ...
    EMM[t] <- emwbM[t] + emsM[t] 
    NtM[t]<- N1M[t] + NMrep[t]
    # Population size (F&M)
    NT[t] <- NtH[t] + NtM[t]
  }   
  mean.EM <- mean(EMM[1:n.occasions]) 
  ...
  # Growth rate
  for (t in 1:(n.occasions - 1)){
    log(lambda[t]) <- log(NT[t + 1])-log(NT[t])
  }

  # ASSESSING THE FIT OF THE PRODUCTIVITY MODEL
  for (t in 2:(n.occasions)) {
    J.exp[t] <- R[t] * fec[t]              # Expected data
    D.obs[t] <- J[t] * log(J[t]/J.exp[t]) - (J[t] - J.exp[t])

    J.rep[t] ~ T(dpois(R[t] * fec[t]),0.1,)
    D.rep[t] <- J.rep[t] * log(J.rep[t]/J.exp[t]) - (J.rep[t] - J.exp[t])
  }
  Dd.obs <- sum(D.obs[2:(n.occasions)])
  Dd.rep <- sum(D.rep[2:(n.occasions)])
  ...
  for (t in 2:n.occasions){
    C.expF[t] <- FRep[t]                                       # Expected counts
    DssmF.obs[t] <- abs((C.FRep[t] - C.expF[t]) / C.FRep[t])   # Discrepancy measure
  }
  DmapeF.obs <- sum(DssmF.obs[1:n.occasions])

  # 2. Compute fit statistic for replicate data
  # Discrepancy measure: mean absolute error
  for (t in 2:n.occasions){
    C.repF[t] ~ T(dnorm(FRep[t], sd=sigma.y1),0.5,)            # Generate replicate data
    DssmF.rep[t] <- abs((C.repF[t] - C.expF[t]) / C.repF[t])   # Discrepancy measure
  }
  DmapeF.rep <- sum(DssmF.rep[1:n.occasions])
  ...
  # Discrepancy measure: mean absolute error
  for (t in 2:n.occasions){
    C.exp[t] <- NMrep[t]                                       # Expected counts
    DssmM.obs[t] <- abs((C.Mrep [t] - C.exp[t]) / C.Mrep [t])  # Discrepancy measure
  }
  DmapeM.obs <- sum(DssmM.obs[1:n.occasions])

  # 2. Compute fit statistic for replicate data
  # Discrepancy measure: mean absolute error
  for (t in 2:n.occasions){
    C.rep[t] ~ T(dnorm(NMrep[t], sd=sigma.y2),0.5,)            # Generate replicate data
    DssmM.rep[t] <- abs((C.rep[t] - C.exp[t]) / C.rep[t])    # Discrepancy measure
  }
  DmapeM.rep <- sum(DssmM.rep[1:n.occasions])
  ...
  # GOF for females dead-recovery data: Freeman-Tukey test statistics
  # Wild-born
  for (t in 1:((n.occasions-1)*ns1)){
    # Simulated m-arrays
    marr.pred1F[t,1:(n.occasions *ns1-(ns1-1))] ~ dmulti(pi1F[t,1:(n.occasions*ns1-(ns1-1))], released1F[t])
    # Expected values and test statistics
    for (j in 1:(n.occasions *ns1-(ns1-1))){
      marr.E1F[t,j] <- pi1F[t,j] * released1F[t]
      E.org1F[t,j] <- pow((pow(marrF[t,j,1], 0.5) - pow(marr.E1F[t,j], 0.5)), 2)
      E.new1F[t,j] <- pow((pow(marr.pred1F[t,j], 0.5) - pow(marr.E1F[t,j], 0.5)), 2)
    } #j
  } #t
  fit.DR1F <- sum(E.org1F[1:((n.occasions-1)*ns1),1:(n.occasions *ns1-(ns1-1))])
  fitN.DR1F <- sum(E.new1F[1:((n.occasions-1)*ns1),1:(n.occasions *ns1-(ns1-1))])
  # Released
  for (t in 1:((n.occasions-1)*ns1)){
    # Simulated m-arrays
    marr.pred2F[t,1:(n.occasions *ns1-(ns1-1))] ~ dmulti(pi2F[t,1:(n.occasions*ns1-(ns1-1))], released2F[t])
    # Expected values and test statistics
    for (j in 1:(n.occasions *ns1-(ns1-1))){
      marr.E2F[t,j] <- pi2F[t,j] * released2F[t]
      E.org2F[t,j] <- pow((pow(marrF[t,j,2], 0.5) - pow(marr.E2F[t,j], 0.5)), 2)
      E.new2F[t,j] <- pow((pow(marr.pred2F[t,j], 0.5) - pow(marr.E2F[t,j], 0.5)), 2)
    } #j
  } #t
  fit.DR2F <- sum(E.org2F[1:((n.occasions-1)*ns1),1:(n.occasions *ns1-(ns1-1))])
  fitN.DR2F <- sum(E.new2F[1:((n.occasions-1)*ns1),1:(n.occasions *ns1-(ns1-1))])
  ...
  # GOF for males dead-recovery data: Freeman-Tukey test statistics
  # Wild-born
  for (t in 1:((n.occasions-1)*ns2)){
    # Simulated m-arrays
    marr.pred1M[t,1:(n.occasions *ns2-(ns2-1))] ~ dmulti(pi1M[t,1:(n.occasions*ns2-(ns2-1))], released1M[t])
    # Expected values and test statistics
    for (j in 1:(n.occasions *ns2-(ns2-1))){
      marr.E1M[t,j] <- pi1M[t,j] * released1M[t]
      E.org1M[t,j] <- pow((pow(marrM[t,j,1], 0.5) - pow(marr.E1M[t,j], 0.5)), 2)
      E.new1M[t,j] <- pow((pow(marr.pred1M[t,j], 0.5) - pow(marr.E1M[t,j], 0.5)), 2)
    } #j
  } #t
  fit.DR1M <- sum(E.org1M[1:((n.occasions-1)*ns2),1:(n.occasions *ns2-(ns2-1))])
  fitN.DR1M <- sum(E.new1M[1:((n.occasions-1)*ns2),1:(n.occasions *ns2-(ns2-1))])
  # Released
  for (t in 1:((n.occasions-1)*ns2)){
    # Simulated m-arrays
    marr.pred2M[t,1:(n.occasions *ns2-(ns2-1))] ~ dmulti(pi2M[t,1:(n.occasions*ns2-(ns2-1))], released2M[t])
    # Expected values and test statistics
    for (j in 1:(n.occasions *ns2-(ns2-1))){
      marr.E2M[t,j] <- pi2M[t,j] * released2M[t]
      E.org2M[t,j] <- pow((pow(marrM[t,j,2], 0.5) - pow(marr.E2M[t,j], 0.5)), 2)
      E.new2M[t,j] <- pow((pow(marr.pred2M[t,j], 0.5) - pow(marr.E2M[t,j], 0.5)), 2)
    } #j
  } #t
  fit.DR2M <- sum(E.org2M[1:((n.occasions-1)*ns2),1:(n.occasions *ns2-(ns2-1))])
  fitN.DR2M <- sum(E.new2M[1:((n.occasions-1)*ns2),1:(n.occasions *ns2-(ns2-1))])  
...
# INITIAL VALUES
set.seed(1961)
str(Inits   <-     list(mean.mr1F     =   runif(1,0.01,1.0),
                        ...
                        mean.mb1F     =   runif(1,0.01,1.0),
                        mean.mb2F     =   runif(1,0.01,1.0),
                        mean.mb3F     =   runif(1,0.01,1.0),
                        mean.mb1M     =   runif(1,0.01,1.0),
                        mean.mb2M     =   runif(1,0.01,1.0),
                        mean.mb3M     =   runif(1,0.01,1.0),
                        ...
                        sigma.mb1F    =   runif(1,0.00,3.0),
                        sigma.mb2F    =   runif(1,0.00,2.0),
                        sigma.mb3F    =   runif(1,0.00,3.0),                        
                        sigma.mb1M    =   runif(1,0.00,2.0),						
                        sigma.mb2M    =   runif(1,0.00,2.0),
                        sigma.mb3M    =   runif(1,0.00,3.0),
...
params <- c('Dd.obs','Dd.rep',
            'DmapeF.obs','DmapeF.rep', 
            'DmapeM.obs','DmapeM.rep', 
            'fit.DR1F','fitN.DR1F','fit.DR2F','fitN.DR2F',
            'fit.DR1M','fitN.DR1M','fit.DR2M','fitN.DR2M', 
            's1F','rk1F','b1F','mean.mr1F','mean.mb1F','sigma.mr1F','sigma.mb1F',
            's1M','rk1M','b1M','mean.mr1M','mean.mb1M','sigma.mr1M','sigma.mb1M',
            's2F','s3F','alpha',
            's2M','s3M','p','alpha',
            'rk2F','rk3F','b2F','b3F',
            'rk2M','rk3M','b2M','b3M',
            'mean.mr2F','mean.mr3F','mean.mb2F','mean.mb3F',
            'mean.mr2M','mean.mr3M','mean.mb2M','mean.mb3M',
            'delta','fec','mean.alpha','mean.fec','xi','di',
            'mean.rF','mean.rM','mean.FF','mean.FM','mean.p',
            'mean.EF','mean.EM',
            'mean.delta','mean.nonPhiF','mean.nonPhiM','lambda',
            'NtH','NtM','NT','FRep','N2wb','N2Rs',
            'N1','N2','F2','N2T','B3','N1M','N2M','N3M','IF','IM',
            'sigma.mr2F','sigma.mr3F','sigma.mb2F','sigma.mb3F',
            'sigma.mr2M','sigma.mr3M','sigma.mb2M','sigma.mb3M',
...
conf<-configureMCMC(Rmodel, monitors = params, useConjugacy=FALSE, enableWAIC = TRUE, thin=5)
